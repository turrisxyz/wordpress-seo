...
const createUpdater = () => {
	...
	return ( [ content, locale ] ) => {
		...
		window.YoastSEO.analysis.worker.runResearch( "readingTime", paper ).then( response => setEstimatedReadingTime( response.result ) );
		window.YoastSEO.analysis.worker.runResearch( "getFleschReadingScore", paper ).then( response => setFleschReadingEase( response.result ) );
		window.YoastSEO.analysis.worker.runResearch( "wordCountInText", paper ).then( response => setWordCount( response.result ) );
	};
};
...
const initialize = () => {
	...
	// Safety check for the analysis worker' research method.
	if ( ! has( window, "YoastSEO.analysis.worker.runResearch" ) ) {
		console.warn( "Insights can not function without the analysis worker" );
		return;
	}
